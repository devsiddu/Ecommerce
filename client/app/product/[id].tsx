...
const ProductDetails = () => {
  ...
  const [selecedSize, setSelectedSize] = useState<string | null>(null);
            ...
            <>
              ...
              <View className="flex-row gap-3 mb-6 flex-wrap">
                {product.sizes.map((size) => (
                  <TouchableOpacity key={size} onPress={() => setSelectedSize(size)} className={`w-12 h-12 rounded-full items-center justify-center border ${selecedSize === size ? 'bg-primary border-primary' : "bg-white border-gray-100"}`} >
                    <Text className={`text-sm font-medium ${selecedSize === size ? "text-white" : "text-primary"}`}>
                      {size}
                    </Text>
                  </TouchableOpacity>
                ))}
