      ...
      >
        {!user ? (
          ...
          <View className='items-center w-full'>
            ...
            <TouchableOpacity onPress={() => router.push("/signin")} className='bg-primary w-3/5 py-3 rounded-full items-center shadow-lg'>
          ...
          <>
            ...
            <View className='items-center mb-8'>
              ...
              {/* admin pannel button if use is admin */}
              {user.publicMetadata?.role === "admin" && (
                <TouchableOpacity onPress={() => router.push("/admin")} className='mt-4
                 bg-primary px-6 py-2 rounded-full'>
            ...
            <View className='bg-white rounded-xl border border-gray-100/75 p-2 mb-4'>
              {PROFILE_MENU.map((item, index) => (
                <TouchableOpacity
                  ...
                  key={item.id} className={`flex-row items-center p-4 ${index !== PROFILE_MENU.length - 1 ? "border-b border-gray-100" : ""}`}>
                  <View className='size-10 bg-surface rounded-full items-center justify-center mr-4'>
                    <Ionicons
