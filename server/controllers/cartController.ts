...
export const getCart = async (req: Request, res: Response,) => {
    try {
        ...
        let cart = await Cart.findOne({ user: req.user.id }).populate("items.product", "name image price stock");
...
export const addToCart = async (req: Request, res: Response,) => {
    try {
        ...
        if (product.stock < quantity) {
            return res.status(500).json({ success: false, message: "Insufficient stock" });
        }
